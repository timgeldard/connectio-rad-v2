...
| 2.5 | Quality decision source documented and blocked until QM evidence verified | 🔶 | `_derive_quality_status` returns `pending` (QI stock > 0) or `unknown` only. `accepted`/`rejected`/`conditional` require a verified QM usage-decision field that is not in the current query. Tests prove conservative values are enforced. Blocker documented in adapter docstring. Implementation plan in `quality-decision-source-plan.md`. **Schema + grain + join key verified (2026-05-21):** object `gold_inspection_usage_decision` (13 cols, 15.47M rows, grain = INSPECTION_LOT_ID + USAGE_DECISION_COUNTER, 0 duplicates); join to `gold_inspection_lot` confirmed; UAT candidate (20052009 / 0008602411 / C061) matched to lot 030005059533. 9 distinct usage-decision codes observed: A (90.3%), AE (7.5%), AC (1.2%), R (0.6%), ACE, RE, A9, RR, '' empty (269 rows). **Governance gate ACTIVE:** QM process owner must confirm code-to-release-status mapping in writing before any accepted/released/rejected display. TRACE-P1-012. |
