...
- **QM Usage-Decision Verified, Broader Pack Pending**: `qm-usage-decision-source-verification.md` verified `gold_inspection_usage_decision` schema (13 columns), grain (`INSPECTION_LOT_ID + USAGE_DECISION_COUNTER`), inspection-lot join, and 9 raw usage-decision codes via Databricks CLI on 2026-05-21 (TRACE-P1-012). All 9 usage-decision codes have a governed mapping (confirmed 2026-05-21). The broader `quality-databricks-source-verification.md` pack for inspection-lot, MIC result, specification, CoA-like, and deviation objects remains pending — no SQL has been run against those objects.
