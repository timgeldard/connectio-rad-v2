...
export const TraceInvestigationContextSchema = z.object({
  ...
  status: z.enum(['open', 'in-progress', 'escalated', 'resolved', 'closed']).describe('[classification: source-field]'),
  severity: z.enum(['low', 'medium', 'high', 'critical']).describe('[classification: source-field]'),
...
export const BatchHeaderSummarySchema = z.object({
  ...
  plantId: z.string().describe('[classification: source-field]'),
  plantName: z.string().describe('[classification: source-field]'),
  batchStatus: z.enum(['active', 'archived', 'blocked', 'deleted', 'unknown']).describe('[classification: source-field]'),
  ...
  stockStatus: z.enum(['unrestricted', 'quality-inspection', 'blocked', 'restricted', 'returns', 'transit']).describe('[classification: application-heuristic]'),
  ...
  qualityStatus: z.enum(['accepted', 'rejected', 'pending', 'conditional', 'not-applicable', 'unknown']).describe('[classification: application-heuristic]'),
  releaseStatus: z.enum(['released', 'blocked', 'restricted', 'not-released', 'unknown']).describe('[classification: governance-pending]'),
})
...
export const TraceNodeSchema = z.object({
  ...
  type: z.enum([
    ...
    'supplier-lot',
    'process-order',
  ]).optional().describe('[classification: source-field]'),
  materialId: z.string().describe('[classification: source-field]'),
  materialDescription: z.string().describe('[classification: source-field]'),
  batchId: z.string().optional().describe('[classification: source-field]'),
  plantId: z.string().optional().describe('[classification: source-field]'),
  quantity: z.number().optional().describe('[classification: source-field]'),
  uom: z.string().optional().describe('[classification: source-field]'),
  status: z.enum(['resolved', 'unresolved', 'blocked', 'partial']).optional().describe('[classification: source-field]'),
  riskLevel: z.enum(['none', 'low', 'medium', 'high', 'critical']).optional().describe('[classification: application-heuristic]'),
...
export const TraceEdgeSchema = z.object({
  ...
  ]).optional().describe('[classification: application-derived]'),
...
export const MassBalanceMovementSchema = z.object({
  ...
  category: z.enum(['production', 'shipment', 'consumption', 'adjustment']).describe('[classification: application-derived]'),
...
export const CustomerExposureSummarySchema = z.object({
  ...
  highestSeverity: z.enum(['none', 'low', 'medium', 'high', 'critical']).describe('[classification: source-field]'),
  blockedDeliveries: z.number().int().min(0).describe('[classification: source-field]'),
  recallRecommended: z.boolean().describe('[classification: governance-pending]'),
  ...
  maxExposureDepth: z.number().int().min(1).nullable().optional().describe('[classification: source-field]'),
  ...
  deliveryEvidenceSource: z.enum(['lineage', 'inventory-movements']).nullable().optional().describe('[classification: source-field]'),
})
...
export const ProductionHistoryRowSchema = z.object({
  ...
  qualityStatus: z.enum(['pass', 'fail', 'unknown']).describe('[classification: application-heuristic]'),
})
...
export const TraceEventSchema = z.object({
  ...
  severity: z.enum(['info', 'warning', 'critical']).optional().describe('[classification: source-field]'),
})
...
export const CoAReleaseStatusSchema = z.object({
  ...
  releaseStatus: z.enum(['released', 'not-released', 'conditional', 'blocked', 'unknown']).describe('[classification: governance-pending]'),
  usageDecision: z.enum(['accept', 'reject', 'pending', 'conditional', 'not-set']).describe('[classification: source-field]'),
...
export const TraceRiskSignalSchema = z.object({
  ...
  severity: z.enum(['info', 'low', 'medium', 'high', 'critical']).describe('[classification: source-field]'),
  source: z.string().describe('[classification: source-field]'),
  confidence: z.number().min(0).max(1).describe('[classification: application-heuristic]'),
